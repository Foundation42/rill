...
| noise & space | `noise period [octaves n] [seed s]` → 0..1 smooth, STATELESS, f32 inside · `rand [seed s]` → 0..1 per rousing. Seeds default to 0 and are the decorrelator; `rand` and `shuffle` share one generator, `noise` is a lattice hash · `distance a b` · `within a b r` — both need `record{x, y, z}` on BOTH sides, and a missing axis is named |
