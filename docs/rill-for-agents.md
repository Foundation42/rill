...
| events & levels | `pulse period [width w]` → VALUE 1/0 (`every` is the occurrence source; width defaults to period/10) · `once` (first value, then deaf; unpiped `once 1` fires at tick 0 by §3.8) · `toggle` · `tally` · `above on off` (HYSTERESIS). **Levels emit at tick 0** — `above` its level, `toggle` false, `tally` 0 — while crossings (`dropped_below`/`rose_above`/`edge`) baseline SILENTLY |
