...
what it emits** — value or occurrence. A bare `→o` is a source. This is
...
| family | operators |
|---|---|
| flow | `select <cond> <a> <b>` · `lerp <t> <a> <b>` (`t` is the piped one: `s \| lerp 0.5 1.5`) · `and`/`or`/`not` · `where <in> <pred>` · `partition <in> <pred>` → pass/fail · `changed` · `latch <in> <trigger>` |
...
| temporal | `sample <in> <period>` v→v · `debounce <in> <quiet>` o→o · `throttle <in> <window>` o→o · `cooldown <in> <window>` o→o · `window <in> <span>` v→v (an array) · `stats <in>` v→v (a record) · `delay <in> <by>` o→o · `every <period>` →o (source; fires at mount then per period; skip-forward after gaps) · `arm <in> <off> <on>` / `disarm <in> <off> <on>` o→o |
...
| sinks | `set <path> [<value>]` · `notify <path> [<value>]` · `inc <path> <by>` · `cast <$channel> [<value>] radius <radius> at <pos> [decay <decay>] [to <#tag>]` · `tag <@subject> <#tag>` / `untag …` (ONE tag per call; unpiped = once at tick 0; membership is a SET — twice is once, only transitions speak) |
