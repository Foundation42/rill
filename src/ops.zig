...
/// **The seventeen tags**, each a short noun and a sentence — what a palette
/// filters on and what its tooltip says (2026-09-09, on Christian's *"We are
/// going to need a palette… With functional groups"*, then his reframe:
/// *"I guess we could view them as filters huh… Think of them as #tags."*).
///
/// Thirteen of these are somebody's HOME — the first tag an operator
/// declares, which is where a grouped listing files it. Four are pure
/// cross-cuts (`constant`, `gate`, `oscillator`, `random`) that no home tree
/// could express, and `time` is both: home to the rate gates, and carried by
/// every word whose behaviour is a function of fed time.
///
/// Names are single nouns on purpose, which is Christian's own register:
/// Blade3D's `OperatorGroup` list reads `Logical`, `Curve`, `Noise`,
/// `FlowControl` — not "logic and comparison", not "select/gate". One word
/// where one word does it, and the sentence carries the rest. His are plural
/// (`Constants`, `Oscillators`); these are singular, because a tag reads
/// singular: an operator IS `#constant`, it is not `#constants`.
///
/// **The type axis is still not here, and now for a sharper reason.** Under a
/// tag model the axes no longer compete — an op could carry `math` and
/// `number` both — so the old objection dissolved and the honest one showed
/// up behind it: `number` would be true of some sixty of these 109 words, and
/// a filter that matches most of the table filters nothing. Blade3D needed
/// `Float Add` and `Integer Add` because it had one operator per type; rill
/// has ONE `add`, whose ports are `number` and whose output is declared `any`
/// because broadcast makes the answer follow the input. Where a type IS the
/// subject and the set is narrow — `array`, `record`, `space` — it earns a
/// tag, and those three are what the evidence left standing.
///
/// **And nothing enforced is in here.** `class`, `routes`, `row` and the port
/// types are checked by the parser and the mount and REFUSE programs; a tag
/// is descriptive and refuses nothing. A tag that restated one of them would
/// be a second answer free to drift from the first, and moving one in here
/// would quietly degrade a real refusal into a label. A palette wanting
/// "row-legal AND tagged `space`" is asking two questions of two columns,
/// which is fine as long as the registry never confuses them.
///
/// The set is closed for rill's own table by an exhaustive audit in
/// `src/tests.zig` — over EVERY tag on every op, not just the home. A HOST's
/// is not closed, because the registry is open and a host interns its own
/// vocabulary, which is why `OpDef.tags` is strings and not an enum.
pub const TAGS = [_]registry.TagDoc{
    .{ .name = "array", .doc = "many values at once: build one, walk it, fold it, order it" },
    .{ .name = "constant", .doc = "a value that never changes" },
    .{ .name = "contract", .doc = "a shape, promised: once at the mount, or on every value" },
    .{ .name = "curve", .doc = "give it t, it gives you a value along a shape" },
    .{ .name = "envelope", .doc = "a value in motion over fed time: it chases, tweens, rises and falls" },
    .{ .name = "event", .doc = "noticing that something happened, and counting it" },
    .{ .name = "flow", .doc = "which way a value goes, and whether it goes at all" },
    .{ .name = "gate", .doc = "may swallow an arrival: the wave can die here" },
    .{ .name = "logic", .doc = "comparison, and the booleans that combine it" },
    .{ .name = "math", .doc = "arithmetic and the elementary functions" },
    .{ .name = "oscillator", .doc = "goes up and down on its own, or shapes something that does" },
    .{ .name = "random", .doc = "variation you did not author: seeded, and identical on every machine" },
    .{ .name = "record", .doc = "named fields: build one, read one, merge two" },
    .{ .name = "sink", .doc = "where a value leaves the program: the plane, a field, a tag, the log" },
    .{ .name = "source", .doc = "a value out of the clock, a seed, or nothing at all" },
    .{ .name = "space", .doc = "positions and directions: distance, angle, and what is near what" },
    .{ .name = "time", .doc = "fed time: how often, how long, and everything that moves because the clock did" },
};

/// The core set. **Every entry declares its `tags`**, home first, drawn from
/// the seventeen above; the audit refuses anything else. The `//` section
/// comments below are the file's own reading order and are older than the
/// tags; several of them are BEAT labels (history, not category). Where the
/// two disagree, `.tags` is the answer, because it is the one a reader of the
/// registry gets.
const CORE = [_]registry.OpDef{
    // flow
    .{ .name = "select", .tags = &.{"flow"}, .row = rowk.exact(rowk.kernels.select), .inputs = &.{ p.in("cond", Tag.boolean), p.in("a", Tag.any), p.in("b", Tag.any) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "cond ? a : b — all branches exist; one is chosen per tick.", .eval = evalSelect },
    .{ .name = "lerp", .tags = &.{"curve"}, .row = rowk.exact(rowk.kernels.lerp), .inputs = &.{ p.in("t", Tag.number), p.in("a", Tag.number), p.in("b", Tag.number) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "a + (b - a) * t, t PIPED — `s | lerp 0.5 1.5` is s, lerped between 0.5 and 1.5.", .eval = evalLerp },
    .{ .name = "and", .tags = &.{"logic"}, .row = rowk.exact(rowk.kernels.andK), .inputs = &.{ p.bc("a", Tag.boolean), p.bc("b", Tag.boolean) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Boolean and — the conjunction idiom's other half: `dark | and calm | …`.", .eval = boolOp(fAnd) },
    .{ .name = "or", .tags = &.{"logic"}, .row = rowk.exact(rowk.kernels.orK), .inputs = &.{ p.bc("a", Tag.boolean), p.bc("b", Tag.boolean) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Boolean or.", .eval = boolOp(fOr) },
    .{ .name = "not", .tags = &.{"logic"}, .row = rowk.exact(rowk.kernels.not), .inputs = &.{p.bc("a", Tag.boolean)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Boolean not.", .eval = unBool() },
    ...
    .{ .name = "edge", .tags = &.{"event"}, .inputs = &.{p.in("in", Tag.boolean)}, .outputs = &.{p.occ("out", Tag.boolean)}, .routes = .anywhere, .help = "Fire on the false→true transition.", .class = .reads, .eval = evalEdge },
    // temporal — durations are 5s / 250ms / 2m / 3f literals; time is fed, never read
    .{ .name = "sample", .tags = &.{"time", "gate"}, .inputs = &.{ p.in("in", Tag.any), p.in("period", Tag.duration) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "At most one emission per period, latest value wins — leading edge immediate, trailing edge via the wheel.", .class = .reads, .eval = evalSample },
    .{ .name = "debounce", .tags = &.{"time", "gate"}, .inputs = &.{ p.occ("in", Tag.any), p.in("quiet", Tag.duration) }, .outputs = &.{p.occ("out", Tag.any)}, .routes = .anywhere, .help = "Pass only after a quiet period; storms collapse to their last edge.", .class = .reads, .eval = evalDebounce },
    .{ .name = "throttle", .tags = &.{"time", "gate"}, .inputs = &.{ p.occ("in", Tag.any), p.in("window", Tag.duration) }, .outputs = &.{p.occ("out", Tag.any)}, .routes = .anywhere, .help = "First occurrence passes, the rest are eaten for the window.", .class = .reads, .eval = evalRateGate },
    .{ .name = "cooldown", .tags = &.{"time", "gate"}, .inputs = &.{ p.occ("in", Tag.any), p.in("window", Tag.duration) }, .outputs = &.{p.occ("out", Tag.any)}, .routes = .anywhere, .help = "Pass one, then deaf for the window — triggered, now get out of the way.", .class = .reads, .eval = evalRateGate },
    .{ .name = "window", .tags = &.{"time"}, .inputs = &.{ p.in("in", Tag.any), p.in("span", Tag.duration) }, .outputs = &.{p.val("out", Tag.array)}, .routes = .anywhere, .help = "Rolling buffer over fed time, emitted as an array; entries age out on schedule even when the input is quiet.", .class = .reads, .eval = evalWindow },
    .{ .name = "stats", .tags = &.{"array"}, .inputs = &.{p.in("in", Tag.array)}, .outputs = &.{p.val("out", Tag.record)}, .routes = .anywhere, .help = "{max, mean, min, n, stddev} over a numeric array; empty in ⇒ zeros with n = 0.", .eval = evalStats },
    .{ .name = "delay", .tags = &.{"time"}, .inputs = &.{ p.occ("in", Tag.any), p.in("by", Tag.duration) }, .outputs = &.{p.occ("out", Tag.any)}, .routes = .anywhere, .help = "Emit each occurrence `by` later; same-tick maturities collapse to the newest.", .class = .reads, .eval = evalDelay },
    .{ .name = "every", .tags = &.{"source", "time"}, .inputs = &.{p.in("period", Tag.duration)}, .outputs = &.{p.occ("out", Tag.boolean)}, .routes = .anywhere, .help = "Occurrence source on a cadence: fires at mount, then once per period of fed time. `every 1f { cast … }` is the standing-caster idiom.", .class = .reads, .eval = evalEvery },
    ...
    .{ .name = "arm", .tags = &.{"flow", "gate"}, .inputs = &.{ p.optOcc("in", Tag.any), p.kwOptOcc("off", Tag.any), p.kwOptOcc("on", Tag.any) }, .outputs = &.{p.occ("out", Tag.any)}, .routes = .anywhere, .help = "Latch gate, initially open: pass occurrences while armed; `off` closes, `on` re-opens (on wins a tie).", .class = .reads, .eval = gateEval(true) },
    .{ .name = "disarm", .tags = &.{"flow", "gate"}, .inputs = &.{ p.optOcc("in", Tag.any), p.kwOptOcc("off", Tag.any), p.kwOptOcc("on", Tag.any) }, .outputs = &.{p.occ("out", Tag.any)}, .routes = .anywhere, .help = "Latch gate, initially closed: silent until `on` arms it; `off` closes again (on wins a tie).", .class = .reads, .eval = gateEval(false) },
    ...
    .{ .name = "clock", .tags = &.{"source", "time"}, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Fed real time in seconds since mount, as a value. Re-evaluates every tick — anything downstream does too.", .class = .reads, .ticks = true, .eval = timeSource(false) },
    .{ .name = "frame", .tags = &.{"source", "time"}, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Fed frame count since mount, as a value. Re-evaluates every frame — anything downstream does too.", .class = .reads, .ticks = true, .eval = timeSource(true) },
    .{ .name = "wave", .tags = &.{"curve", "oscillator"}, .inputs = &.{ p.in("t", Tag.number), p.oneOf("shape", &WAVE_SHAPES), p.in("period", Tag.duration) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Shape piped time into 0..1 — `clock | wave sine 4s`; shapes: sine, tri, saw, square. Pure: same t, same answer.", .eval = evalWave },
    .{ .name = "lfo", .tags = &.{"source", "oscillator", "time"}, .inputs = &.{ p.oneOf("shape", &WAVE_SHAPES), p.in("period", Tag.duration), p.kwOpt("phase", Tag.number) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Modulation source in 0..1 — `lfo sine 4s [phase 0.25]`. The same waveform as `clock | wave`, in one node.", .class = .reads, .ticks = true, .eval = evalLfo },
    .{ .name = "ease", .tags = &.{"envelope", "time"}, .inputs = &.{ p.in("in", Tag.number), p.in("tau", Tag.duration), p.kwOpt("up", Tag.duration), p.kwOpt("down", Tag.duration) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Chase the input with time constant `tau`; `up`/`down` make it asymmetric. Stops inside epsilon of the target — it never snaps.", .class = .reads, .ticks = true, .eval = evalEase },
    .{ .name = "ramp", .tags = &.{"envelope", "time"}, .inputs = &.{ p.in("in", Tag.number), p.in("over", Tag.duration), p.kwOpt("from", Tag.number) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Tween linearly to each new target over `over`; retargeting starts from where it is. `from` gives the FIRST tween a start, which is the mount fade — `once 1 | ramp 2s from 0`. The last frame emits the target exactly.", .class = .reads, .ticks = true, .eval = evalRamp },
    .{ .name = "hold", .tags = &.{"time", "gate"}, .inputs = &.{ p.in("in", Tag.number), p.in("for", Tag.duration) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Sample-and-hold: take a value, then ignore changes for `for`. Does not tick — ignored values are gone.", .class = .reads, .eval = evalHold },
    .{ .name = "diff", .tags = &.{"envelope", "time"}, .inputs = &.{p.in("in", Tag.number)}, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Rate of change per second from the previous sample. First observation baselines silently; ticks while moving, stops at zero.", .class = .reads, .ticks = true, .eval = evalDiff },
    .{ .name = "integrate", .tags = &.{"envelope", "time"}, .inputs = &.{ p.in("in", Tag.number), p.kwIn("max", Tag.number) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Running sum over fed time, clamped to +/-max. The clamp is required — unbounded state is a corpse that rides every dump.", .class = .reads, .ticks = true, .eval = evalIntegrate },
    .{ .name = "range", .tags = &.{"curve"}, .row = rowk.exact(rowk.kernels.range), .inputs = &.{ p.in("t", Tag.number), p.in("lo", Tag.number), p.in("hi", Tag.number) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Map 0..1 onto lo..hi, CLAMPING outside it — the exit from the unit interval. `lerp` is the same arithmetic that extrapolates.", .eval = evalRange },
    .{ .name = "over", .tags = &.{"curve"}, .row = rowk.exact(rowk.kernels.over), .inputs = &.{ p.in("t", Tag.number), p.in("span", Tag.number), p.in("curve", Tag.array) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Sample a curve of EVENLY-SPACED knots at t/span, clamped to the ends — `row.age | over row.life [1, 0.7, 0]` is full size at birth, 0.7 halfway, nothing at death. `range` is the two-knot case. Knots may be numbers or records (a record ramp is a colour ramp); one knot is a constant. A zero span refuses.", .eval = evalOver },
    .{ .name = "shape", .tags = &.{"curve"}, .inputs = &.{ p.in("t", Tag.number), p.oneOf("curve", &SHAPE_CURVES) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Ease a 0..1 value into 0..1 — curves: linear, smooth, in, out, inout. Input clamps.", .eval = evalShape },
    ...
    // math
    .{ .name = "add", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.add), .inputs = &.{ p.bc("a", Tag.number), p.bc("b", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "a + b.", .eval = binMath(fAdd) },
    .{ .name = "sub", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.sub), .inputs = &.{ p.bc("a", Tag.number), p.bc("b", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "a - b.", .eval = binMath(fSub) },
    .{ .name = "mul", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.mulK), .inputs = &.{ p.bc("a", Tag.number), p.bc("b", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "a * b.", .eval = binMath(fMul) },
    .{ .name = "div", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.div), .inputs = &.{ p.bc("a", Tag.number), p.bc("b", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "a / b (IEEE; division by zero yields ±inf).", .eval = binMath(fDiv) },
    .{ .name = "min", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.min), .inputs = &.{ p.bc("a", Tag.number), p.bc("b", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "The smaller of a and b.", .eval = binMath(fMin) },
    .{ .name = "max", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.max), .inputs = &.{ p.bc("a", Tag.number), p.bc("b", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "The larger of a and b.", .eval = binMath(fMax) },
    .{ .name = "clamp", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.clamp), .inputs = &.{ p.in("in", Tag.number), p.in("lo", Tag.number), p.in("hi", Tag.number) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Clamp `in` to [lo, hi].", .eval = evalClamp },
    .{ .name = "abs", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.abs), .inputs = &.{p.bc("in", Tag.number)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Absolute value.", .eval = unMath(fAbs) },
    .{ .name = "floor", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.floor), .inputs = &.{p.bc("in", Tag.number)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Round toward −inf.", .eval = unMath(fFloor) },
    .{ .name = "round", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.round), .inputs = &.{p.bc("in", Tag.number)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Round to nearest.", .eval = unMath(fRound) },
    // math completions (beat 1b) — every one broadcasts, being minted by the
    // same helpers as `add`.
    .{ .name = "sin", .tags = &.{"math"}, .inputs = &.{p.bc("in", Tag.number)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Sine, radians.", .eval = unMath(fSin) },
    .{ .name = "cos", .tags = &.{"math"}, .inputs = &.{p.bc("in", Tag.number)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Cosine, radians.", .eval = unMath(fCos) },
    .{ .name = "tan", .tags = &.{"math"}, .inputs = &.{p.bc("in", Tag.number)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Tangent, radians.", .eval = unMath(fTan) },
    .{ .name = "sqrt", .tags = &.{"math"}, .inputs = &.{p.bc("in", Tag.number)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Square root (IEEE; a negative yields nan).", .eval = unMath(fSqrt) },
    .{ .name = "exp", .tags = &.{"math"}, .inputs = &.{p.bc("in", Tag.number)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "e to the input.", .eval = unMath(fExp) },
    .{ .name = "log", .tags = &.{"math"}, .inputs = &.{p.bc("in", Tag.number)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Natural log (IEEE; zero yields -inf, a negative yields nan).", .eval = unMath(fLog) },
    .{ .name = "ceil", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.ceil), .inputs = &.{p.bc("in", Tag.number)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Round toward +inf — `floor`'s missing mirror.", .eval = unMath(fCeil) },
    .{ .name = "sign", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.sign), .inputs = &.{p.bc("in", Tag.number)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "-1, 0 or 1 (nan stays nan).", .eval = unMath(fSign) },
    .{ .name = "fract", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.fract), .inputs = &.{p.bc("in", Tag.number)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Fractional part, always in [0, 1) — `fract -0.25` is 0.75.", .eval = unMath(fFract) },
    .{ .name = "pow", .tags = &.{"math"}, .inputs = &.{ p.bc("a", Tag.number), p.bc("b", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "a to the power b — `x | pow 2` is x squared.", .eval = binMath(fPow) },
    .{ .name = "mod", .tags = &.{"math"}, .row = rowk.exact(rowk.kernels.mod), .inputs = &.{ p.bc("a", Tag.number), p.bc("b", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Floored modulo: the sign follows the divisor, so `-90 | mod 360` is 270.", .eval = binMath(fMod) },
    .{ .name = "atan2", .tags = &.{"math"}, .inputs = &.{ p.in("y", Tag.number), p.in("x", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Angle of (x, y) in radians, y PIPED — `dy | atan2 dx`.", .eval = binMath(fAtan2) },
    .{ .name = "pi", .tags = &.{"source", "constant"}, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "3.14159… — emitted once at mount.", .eval = constant(std.math.pi) },
    .{ .name = "tau", .tags = &.{"source", "constant"}, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "2*pi — a whole turn, emitted once at mount.", .eval = constant(std.math.tau) },
    // comparators
    .{ .name = "=", .tags = &.{"logic"}, .row = rowk.exact(rowk.kernels.eq), .inputs = &.{ p.in("a", Tag.any), p.in("b", Tag.any) }, .outputs = &.{p.val("out", Tag.boolean)}, .routes = .anywhere, .help = "Equality (numeric across int/float; byte-wise otherwise).", .eval = evalEq },
    .{ .name = "!=", .tags = &.{"logic"}, .row = rowk.exact(rowk.kernels.ne), .inputs = &.{ p.in("a", Tag.any), p.in("b", Tag.any) }, .outputs = &.{p.val("out", Tag.boolean)}, .routes = .anywhere, .help = "Inequality.", .eval = evalNe },
    .{ .name = "<", .tags = &.{"logic"}, .row = rowk.exact(rowk.kernels.lt), .inputs = &.{ p.bc("a", Tag.number), p.bc("b", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "a < b.", .eval = cmpOp(fLt) },
    .{ .name = "<=", .tags = &.{"logic"}, .row = rowk.exact(rowk.kernels.le), .inputs = &.{ p.bc("a", Tag.number), p.bc("b", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "a <= b.", .eval = cmpOp(fLe) },
    .{ .name = ">", .tags = &.{"logic"}, .row = rowk.exact(rowk.kernels.gt), .inputs = &.{ p.bc("a", Tag.number), p.bc("b", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "a > b.", .eval = cmpOp(fGt) },
    .{ .name = ">=", .tags = &.{"logic"}, .row = rowk.exact(rowk.kernels.ge), .inputs = &.{ p.bc("a", Tag.number), p.bc("b", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "a >= b.", .eval = cmpOp(fGe) },
    ...
    .{ .name = "array", .tags = &.{"array"}, .variadic = true, .outputs = &.{p.val("out", Tag.array)}, .routes = .anywhere, .help = "Array construction [ a, b, c ] — a live tuple with positions instead of names.", .eval = evalArray },
    .{ .name = "nth", .tags = &.{"array"}, .inputs = &.{ p.in("in", Tag.array), p.in("i", Tag.number) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "The i-th element, 0-based — `window 5s | nth 0`. Out of range is an error, never a clamp.", .eval = indexEval(0, 1) },
    .{ .name = "choose", .tags = &.{"array"}, .inputs = &.{ p.in("i", Tag.number), p.in("of", Tag.array) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "`nth` with the index piped — `plane.time.band | choose [0.2, 1, 0.6, 0.05]`. Pick one of these.", .eval = indexEval(1, 0) },
    // tier 2, beat 4b — noise, randomness, space.
    .{ .name = "noise", .tags = &.{"source", "random", "time"}, .inputs = &.{ p.in("period", Tag.duration), p.kwOpt("octaves", Tag.number), p.kwOpt("seed", Tag.number) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .class = .reads, .ticks = true, .help = "Smooth noise in 0..1 over fed time — `noise 80ms` flickers, `noise 20s` drifts. Stateless, seeded (default 0), bit-identical across machines.", .eval = evalNoise },
    .{ .name = "rand", .tags = &.{"source", "random"}, .inputs = &.{ p.occ("in", Tag.any), p.kwOpt("seed", Tag.number) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .class = .reads, .help = "White: a fresh value in 0..1 per rousing — `plane.trigger | rand | mul 4 | floor`. Same generator as `shuffle`; seed defaults to 0.", .eval = evalRand },
    .{ .name = "distance", .tags = &.{"space"}, .inputs = &.{ p.in("a", Tag.record), p.in("b", Tag.record) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Distance between two positions — both record{x, y, z}; a missing axis is named.", .eval = evalDistance },
    .{ .name = "dot", .tags = &.{"space"}, .inputs = &.{ p.in("a", Tag.record), p.in("b", Tag.record) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Scalar product of two positions or directions — both record{x, y, z}. How much of a lies along b.", .eval = evalDot },
    .{ .name = "within", .tags = &.{"space"}, .inputs = &.{ p.in("a", Tag.record), p.in("b", Tag.record), p.in("r", Tag.number) }, .outputs = &.{p.val("out", Tag.boolean)}, .routes = .anywhere, .help = "Is a within r of b? Both record{x, y, z}.", .eval = evalWithin },
    .{ .name = "angle", .tags = &.{"space"}, .inputs = &.{ p.in("a", Tag.record), p.in("b", Tag.record) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Angle between two directions, radians 0..π — both record{x, y, z}. A zero-length vector refuses: no direction, no angle.", .eval = evalAngle },
    .{ .name = "inside", .tags = &.{"space"}, .inputs = &.{ p.in("p", Tag.record), p.in("min", Tag.record), p.in("max", Tag.record) }, .outputs = &.{p.val("out", Tag.boolean)}, .routes = .anywhere, .help = "Is p inside the axis-aligned box from min to max? Bounds inclusive — the wall counts, like `within`'s sphere. An inverted box is empty and answers false.", .eval = evalInside },
    .{ .name = "cross", .tags = &.{"space"}, .inputs = &.{ p.in("a", Tag.record), p.in("b", Tag.record) }, .outputs = &.{p.val("out", Tag.record)}, .routes = .anywhere, .help = "Vector product of two directions — record{x, y, z} out, right-handed: x cross y is z.", .eval = evalCross },
    // tier 2, beat 4a — events and levels.
    .{ .name = "pulse", .tags = &.{"source", "oscillator", "time"}, .inputs = &.{ p.in("period", Tag.duration), p.kwOpt("width", Tag.duration) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .class = .reads, .ticks = true, .help = "Value source: 1 for `width`, else 0, once per period — `pulse 1s [width 100ms]`. Width defaults to a tenth of the period. `every` is the occurrence source.", .eval = evalPulse },
    .{ .name = "once", .tags = &.{"flow", "gate"}, .inputs = &.{p.occ("in", Tag.any)}, .outputs = &.{p.occ("out", Tag.any)}, .routes = .anywhere, .class = .reads, .help = "Pass the first value, then deaf until remount — `once 1 | ramp 2s` fires at mount; piped, it passes the first arrival.", .eval = evalOnce },
    .{ .name = "toggle", .tags = &.{"event"}, .inputs = &.{p.occ("in", Tag.any)}, .outputs = &.{p.val("out", Tag.boolean)}, .routes = .anywhere, .class = .reads, .help = "Flip a boolean on each arrival. Emits its initial false at mount and flips from the next arrival on.", .eval = evalToggle },
    .{ .name = "tally", .tags = &.{"event"}, .inputs = &.{p.occ("in", Tag.any)}, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .class = .reads, .help = "Running count of arrivals, as a value. Emits 0 at mount; does not survive remount (remount is restart).", .eval = evalTally },
    // envelopes campaign, 2026-08-26 — the register family's missing half
    .{ .name = "kick", .tags = &.{"envelope", "time"}, .inputs = &.{ p.occ("in", Tag.any), p.in("attack", Tag.duration), p.in("decay", Tag.duration) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .class = .reads, .ticks = true, .help = "One-shot envelope from an occurrence — `plane.events.hit | kick 20ms 400ms`. Rises to 1 over `attack`, falls to 0 over `decay`, stops. A retrigger restarts from the CURRENT level, never from zero.", .eval = evalKick },
    .{ .name = "adsr", .tags = &.{"envelope", "time"}, .inputs = &.{ p.in("in", Tag.boolean), p.in("attack", Tag.duration), p.in("decay", Tag.duration), p.in("sustain", Tag.number), p.in("release", Tag.duration) }, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .class = .reads, .ticks = true, .help = "Gate in, envelope out — `plane.input.key | adsr 10ms 80ms 0.7 400ms`. Rise, decay to `sustain` while the gate holds, release when it drops. A held sustain costs nothing.", .eval = evalAdsr },
    .{ .name = "above", .tags = &.{"event"}, .inputs = &.{ p.in("in", Tag.number), p.in("on", Tag.number), p.in("off", Tag.number) }, .outputs = &.{p.val("out", Tag.boolean)}, .routes = .anywhere, .class = .reads, .help = "Boolean with hysteresis — `above 0.3 0.2` is \"above 0.3, until below 0.2\". Emits its level at mount; this is what stops a threshold chattering.", .eval = hysteresis(false) },
    .{ .name = "below", .tags = &.{"event"}, .inputs = &.{ p.in("in", Tag.number), p.in("on", Tag.number), p.in("off", Tag.number) }, .outputs = &.{p.val("out", Tag.boolean)}, .routes = .anywhere, .class = .reads, .help = "Boolean with hysteresis, falling — `below 0.2 0.3` is \"below 0.2, until above 0.3\". The first number trips and the second releases, same as `above`. Emits its level at mount.", .eval = hysteresis(true) },
    ...
    .{ .name = "map", .tags = &.{"array"}, .inputs = &.{p.in("in", Tag.array)}, .outputs = &.{p.val("out", Tag.array)}, .routes = .anywhere, .class = .reads, .body = 1, .help = "The body once per element, in order — `map (clamp 0 1)`. Keeps the length; use `keep` to drop.", .eval = evalMap },
    .{ .name = "keep", .tags = &.{"array"}, .inputs = &.{p.in("in", Tag.array)}, .outputs = &.{p.val("out", Tag.array)}, .routes = .anywhere, .class = .reads, .body = 1, .help = "The elements a predicate says true for — `keep (> 0)`. Filters ELEMENTS; `where` gates the stream.", .eval = evalKeep },
    .{ .name = "reduce", .tags = &.{"array"}, .inputs = &.{ p.in("in", Tag.array), p.kwOpt("init", Tag.any) }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .class = .reads, .body = 2, .help = "Left fold — `reduce (add) [init 0]`. Accumulator fills the body's first open port, element the second; no init means the first element seeds.", .eval = evalReduce },
    ...
    .{ .name = "sort", .tags = &.{"array"}, .inputs = &.{p.in("in", Tag.array)}, .statics = &.{.{ .name = "desc", .kind = .word, .flag = true, .optional = true }}, .outputs = &.{p.val("out", Tag.array)}, .routes = .anywhere, .class = .reads, .body = 1, .body_kw = "by", .help = "Stable sort — `sort by (.distance) [desc]`. Without `by`, the elements are their own keys. Ties keep their input order.", .eval = evalSort },
    .{ .name = "first", .tags = &.{"array"}, .inputs = &.{p.in("in", Tag.array)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "The leading element — `sort by (.distance) | first`. An empty array ends the wave silently; say absence with `len`.", .eval = endEval(false) },
    .{ .name = "last", .tags = &.{"array"}, .inputs = &.{p.in("in", Tag.array)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "The trailing element — `window 5s | last` is the most recent reading. An empty array ends the wave silently.", .eval = endEval(true) },
    .{ .name = "len", .tags = &.{"array"}, .inputs = &.{p.in("in", Tag.array)}, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "How many elements — `contacts | len | write plane.ui.contacts`. This is how absence is said once `first` goes quiet.", .eval = evalLen },
    .{ .name = "take", .tags = &.{"array"}, .inputs = &.{ p.in("in", Tag.array), p.in("n", Tag.number), p.kwOpt("from", Tag.number) }, .outputs = &.{p.val("out", Tag.array)}, .routes = .anywhere, .help = "At most `n` elements, from `from` — `take 3`. A short array is forgiven; `nth` past the end is not.", .eval = evalTake },
    .{ .name = "transpose", .tags = &.{"array"}, .inputs = &.{p.in("in", Tag.any)}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "AoS ↔ SoA, self-inverse — `{a: [1,2], b: [3,4]}` ↔ `[{a:1,b:3},{a:2,b:4}]`. Ragged input refuses, both sides named.", .eval = evalTranspose },
    .{ .name = "shuffle", .tags = &.{"array", "random"}, .inputs = &.{ p.in("in", Tag.array), p.kwOpt("seed", Tag.number) }, .outputs = &.{p.val("out", Tag.array)}, .routes = .anywhere, .help = "Seeded Fisher–Yates — `shuffle [seed 7] | take 3` is three at random, no repeats. Seed defaults to 0; bit-identical across machines.", .eval = evalShuffle },
    .{ .name = "step", .tags = &.{"flow"}, .inputs = &.{ p.occ("in", Tag.any), p.in("of", Tag.array), p.kwOpt("seed", Tag.number), p.kwOpt("max", Tag.number) }, .statics = &.{ .{ .name = "loop", .kind = .word, .flag = true, .optional = true }, .{ .name = "bounce", .kind = .word, .flag = true, .optional = true }, .{ .name = "reverse", .kind = .word, .flag = true, .optional = true }, .{ .name = "random", .kind = .word, .flag = true, .optional = true }, .{ .name = "shuffle", .kind = .word, .flag = true, .optional = true } }, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .class = .reads, .help = "Step sequencer — `plane.beat | step [60, 64, 67] loop`. Each rousing emits the NEXT element; by default it runs once and the wave ends. Order: sequential (`reverse` walks down) or `random`/`shuffle`; end: `loop`, `bounce`, or stop. `max n` caps emissions.", .eval = evalStep },
    .{ .name = "nearest", .tags = &.{"curve"}, .inputs = &.{ p.in("p", Tag.record), p.in("knots", Tag.array) }, .statics = &.{.{ .name = "loop", .kind = .word, .flag = true, .optional = true }}, .outputs = &.{p.val("out", Tag.number)}, .routes = .anywhere, .help = "Where p is on the curve through the knots, as t in 0..1 — the inverse of `along`. `loop` searches the closed curve, seam included, and answers in 0..1 with the seam at 0. Fewer than two knots refuses; a loop needs three.", .eval = evalNearest },
    .{ .name = "along", .tags = &.{"curve"}, .inputs = &.{ p.in("t", Tag.number), p.in("knots", Tag.array) }, .statics = &.{.{ .name = "loop", .kind = .word, .flag = true, .optional = true }}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Travel a Catmull-Rom curve through the knots as t goes 0..1 — `along [a, b, c]`. Clamps outside 0..1. `loop` closes the curve back to the first knot and wraps t instead — a loop has no ends, and do not repeat the first knot as the last. Fewer than two knots refuses; a loop needs three.", .eval = evalAlong },
    // tier 2, beat 2b — contracts. One shape literal, two promises.
    .{ .name = "expect", .tags = &.{"contract"}, .inputs = &.{p.opt("in", Tag.any)}, .statics = &.{.{ .name = "shape", .kind = .shape }}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .class = .reads, .fails_mount = true, .help = "Assert a shape ONCE, at mount — `expect {id: string, distance: number} [exact]`. A mismatch refuses the mount. Costs nothing afterwards; never falls back to a runtime check.", .eval = evalExpect },
    .{ .name = "match", .tags = &.{"contract"}, .inputs = &.{p.in("in", Tag.any)}, .statics = &.{.{ .name = "shape", .kind = .shape }}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Assert a shape on EVERY value — `match {id: string} [exact]`. A mismatch kills the wave and names the field and both sides.", .eval = evalMatch },
    // records
    .{ .name = "record", .tags = &.{"record"}, .row = rowk.exact(rowk.kernels.record), .variadic = true, .outputs = &.{p.val("out", Tag.record)}, .routes = .anywhere, .help = "Record construction { field: stream, … } — a live tuple with named fields.", .eval = evalRecord },
    .{ .name = "project", .tags = &.{"record"}, .row = rowk.exact(rowk.kernels.project), .inputs = &.{p.in("in", Tag.record)}, .statics = &.{.{ .name = "field", .kind = .word }}, .outputs = &.{p.val("out", Tag.any)}, .routes = .anywhere, .help = "Field access on a record stream (`stats.mana`).", .eval = evalProject },
    .{ .name = "merge", .tags = &.{"record"}, .inputs = &.{ p.in("a", Tag.record), p.in("b", Tag.record) }, .outputs = &.{p.val("out", Tag.record)}, .routes = .anywhere, .help = "Merge two records; b's fields win.", .eval = evalMerge },
    // plane / util
    .{
        .name = "write",
        .tags = &.{"sink"},
